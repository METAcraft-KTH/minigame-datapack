...
All occurrences of `spl` (both uppercase and lowercase) is to be replaced by a unique name for the game, in order to prevent namespace conflicts.
...
- `spl.game` all dummy variables related to the game.
    - `?state` the game's current state.
    - `?timer` variable used to keep track of the gamestate's MAIN timer.
- `spl.config` all constant values referenced by the game. All of them should be defined in [load.mcfunction](/data/spl/function/load.mcfunction).
...
- Additional `spl.*` objectives can be created to suit the game's needs, just make sure they're namespaced with the `spl.` prefix.
...
- `spl.tp.arena` marker in the game arena. Everyone gets TPed here on game start (`function spl:states/pregame/start`)
- `spl.tp.lobby` marker in the main lobby. Everyone gets TPed here on game end (`function spl:exit`)
...
2. The datapack with the corresponding ID (defined in `id spl.config`) runs, starting with the `spl:states/lobby/start` function, since the `?state` variable should be pre-configured to `-1`.
3. It then sets `?state` to `0`, activating `spl:states/lobby/tick`. This function ticks down until the game begins, where it then calls `spl:states/pregame/start`...
...
> [!WARNING]  
> Advancements will trigger even if the current gamestate doesn't match. Always check if `?state spl.game` matches in the reward function, and stop executing if it doesn't match.
>
...
- ALL tags must be namespaced (prefixed) with `spl.`.
