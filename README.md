...
WALLS.tp.arena.r1.red
WALLS.tp.arena.r1.red.deathmatch
WALLS.tp.arena.r1.blue
WALLS.tp.arena.r1.blue.deathmatch
WALLS.tp.arena.r1.green
WALLS.tp.arena.r1.green.deathmatch
WALLS.tp.arena.r1.yellow
WALLS.tp.arena.r1.yellow.deathmatch

WALLS.tp.arena.r2.red
WALLS.tp.arena.r2.red.deathmatch
WALLS.tp.arena.r2.blue
WALLS.tp.arena.r2.blue.deathmatch
...
All occurrences of `WALLS` (both uppercase and lowercase) is to be replaced by a unique name for the game, in order to prevent namespace conflicts.
...
- `WALLS.game` all dummy variables related to the game.
    - `?state` the game's current state.
    - `?timer` variable used to keep track of the gamestate's MAIN timer.
- `WALLS.config` all constant values referenced by the game. All of them should be defined in [load.mcfunction](/data/walls/function/load.mcfunction).
...
- Additional `WALLS.*` objectives can be created to suit the game's needs, just make sure they're namespaced with the `WALLS.` prefix.
...
- `WALLS.tp.arena` marker in the game arena. Everyone gets TPed here on game start (`function walls:states/pregame/start`)
- `WALLS.tp.lobby` marker in the main lobby. Everyone gets TPed here on game end (`function walls:exit`)
...
2. The datapack with the corresponding ID (defined in `id WALLS.config`) runs, starting with the `walls:states/lobby/start` function, since the `?state` variable should be pre-configured to `-1`.
3. It then sets `?state` to `0`, activating `walls:states/lobby/tick`. This function ticks down until the game begins, where it then calls `walls:states/pregame/start`...
...
> [!WARNING]  
> Advancements will trigger even if the current gamestate doesn't match. Always check if `?state WALLS.game` matches in the reward function, and stop executing if it doesn't match.
>
...
- ALL tags must be namespaced (prefixed) with `WALLS.`.
