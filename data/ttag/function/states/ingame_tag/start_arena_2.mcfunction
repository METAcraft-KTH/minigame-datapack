...
scoreboard players set num.bomb ttag.config 25
...
# replace blocks
execute at @n[tag=ttag.arena1.beacon] run setblock ~ ~ ~ red_stained_glass
execute at @n[tag=ttag.arena1.beacon] run fill ~10 ~10 ~10 ~-10 ~-10 ~-10 air replace deepslate_tiles

# set spawn
execute at @n[tag=ttag.arena2.spawn] run setworldspawn ~ ~ ~ 0