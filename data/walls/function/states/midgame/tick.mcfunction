...
execute as @a[scores={GLOBAL.player_left=1..},tag=!admin] run function walls:states/midgame/join
execute as @a[scores={GLOBAL.time_alive=0},tag=!admin] run function walls:states/midgame/while_dead
execute as @a[scores={GLOBAL.time_alive=1},tag=!admin] run function walls:states/midgame/respawn

# increment timer
scoreboard players add ?timer WALLS.game 1
# calculate remaining time
scoreboard players operation #remainingseconds WALLS.game = time.midgame WALLS.config
...
## start game (for real)
execute if score ?timer WALLS.game >= time.midgame WALLS.config run function walls:states/ingame/start.r2
