...
All occurrences of `atc` (both uppercase and lowercase) is to be replaced by a unique name for the game, in order to prevent namespace conflicts.
...
- `atc.game` all dummy variables related to the game.
    - `?state` the game's current state.
    - `?timer` variable used to keep track of the gamestate's MAIN timer.
- `atc.config` all constant values referenced by the game. All of them should be defined in [load.mcfunction](/data/atc/function/load.mcfunction).
...
- Additional `atc.*` objectives can be created to suit the game's needs, just make sure they're namespaced with the `atc.` prefix.
...
When a game starts, it will run the `#atc:init` function tag.
You probably want to hardcode coordinates to forceload in this tag in a separate datapack.
When a game ends, it will run the `#atc:exit` function tag, which is a perfect opportunity to unforceload chunks, enable the next datapack and disable this one.
...
2. The datapack with the corresponding ID (defined in `id atc.config`) runs, starting with the `atc:states/lobby/start` function, since the `?state` variable should be pre-configured to `-1`.
3. It then sets `?state` to `0`, activating `atc:states/lobby/tick`. This function ticks down until the game begins, where it then calls `atc:states/pregame/start`...
...
> [!WARNING]  
> Advancements will trigger even if the current gamestate doesn't match. Always check if `?state atc.game` matches in the reward function, and stop executing if it doesn't match.
>
...
- ALL tags must be namespaced (prefixed) with `atc.`.
