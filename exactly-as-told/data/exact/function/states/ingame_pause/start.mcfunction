...
execute if score ?round.number exact.game matches 4 run gamerule block_drops false
...
execute if score ?round.number exact.game matches 15 at @n[type=marker,tag=exact.tp.arena] run kill @e[type=vex,distance=..100]
