...
scoreboard players set ?state exact.game 0

# initialize timer
execute store result bossbar exact:timer max run scoreboard players get time.lobby exact.config
...
execute as @a[tag=!admin] run function exact:states/lobby/reset_player
