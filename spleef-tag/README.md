...
All occurrences of `spleef` (both uppercase and lowercase) is to be replaced by a unique name for the game, in order to prevent namespace conflicts.
...
- `spleef.game` all dummy variables related to the game.
    - `?state` the game's current state.
    - `?timer` variable used to keep track of the gamestate's MAIN timer.
- `spleef.config` all constant values referenced by the game. All of them should be defined in [load.mcfunction](/data/spleef/function/load.mcfunction).
...
- Additional `spleef.*` objectives can be created to suit the game's needs, just make sure they're namespaced with the `spleef.` prefix.
...
- `spleef.tp.arena` marker in the game arena. Everyone gets TPed here on game start (`function spleef:states/pregame/start`)
- `spleef.tp.lobby` marker in the main lobby. Everyone gets TPed here on game end (`function spleef:exit`)
...
When a game ends, it will run the `#spleef:exit` function tag, which is a perfect opportunity to unforceload chunks, enable the next datapack and disable this one.
...
2. The datapack with the corresponding ID (defined in `id spleef.config`) runs, starting with the `spleef:states/lobby/start` function, since the `?state` variable should be pre-configured to `-1`.
3. It then sets `?state` to `0`, activating `spleef:states/lobby/tick`. This function ticks down until the game begins, where it then calls `spleef:states/pregame/start`...
...
> [!WARNING]  
> Advancements will trigger even if the current gamestate doesn't match. Always check if `?state spleef.game` matches in the reward function, and stop executing if it doesn't match.
>
...
- ALL tags must be namespaced (prefixed) with `spleef.`.
