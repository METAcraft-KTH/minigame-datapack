...
execute as @e[type=marker,tag=mingle.cp,tag=!mingle.cp.disabled] if score @s mingle.temp < ?target_group mingle.state at @s run tag @a[tag=!admin,gamemode=!spectator,distance=..4] add mingle.player.toofew
execute as @e[type=marker,tag=mingle.cp,tag=!mingle.cp.disabled] if score @s mingle.temp = ?target_group mingle.state at @s run tag @a[tag=!admin,gamemode=!spectator,distance=..4] add mingle.player.correct
execute as @e[type=marker,tag=mingle.cp,tag=!mingle.cp.disabled] if score @s mingle.temp > ?target_group mingle.state at @s run tag @a[tag=!admin,gamemode=!spectator,distance=..4] add mingle.player.toomany
