...
- `GAMENAME.tp.arena` marker in the game arena. Everyone gets TPed here on game start (`function gamename:states/pregame/start`)
- `GAMENAME.tp.lobby` marker in the main lobby. Everyone gets TPed here on game end (`function gamename:exit`)
...
When a game ends, it will run the `#gamename:exit` function tag, which is a perfect opportunity to unforceload chunks, enable the next datapack and disable this one.
...
3. It then sets `?state` to `0`, activating `gamename:states/lobby/tick`. This function ticks down until the game begins, where it then calls `gamename:states/pregame/start`...
...
- ALL tags must be namespaced (prefixed) with `GAMENAME.`.
