# ============================================================
# walls:events/on_place_spawner_zombie
# Called by: advancement walls:place_spawner_zombie
...
advancement revoke @s only walls:place_spawner_zombie
execute unless score ?superstate main.state matches 3 run return 0

data modify storage walls:spawner pending set value {spawn:{entity:{id:"minecraft:zombie",Tags:["walls.mob"],equipment:{head:{id:"minecraft:iron_helmet",count:1}},drop_chances:{head:0.0f}},custom_spawn_rules:{block_light_limit:{min_inclusive:0,max_inclusive:15},sky_light_limit:{min_inclusive:0,max_inclusive:15}}},min:200,max:400}
