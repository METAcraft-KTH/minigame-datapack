...
execute if score ?phase tnttag.state matches 1 as @s[tag=!admin] run gamemode adventure

# Outside gameplay rounds, keep players in spectator.
execute unless score ?phase tnttag.state matches 1 as @s[tag=!admin] run gamemode spectator