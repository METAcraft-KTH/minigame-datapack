...
- `SURVIVALGAMES.game` all dummy variables related to the game.
    - `?state` the game's current state.
    - `?timer` variable used to keep track of the gamestate's MAIN timer.
- `SURVIVALGAMES.config` all constant values referenced by the game. All of them should be defined in [load.mcfunction](/data/survivalgames/function/load.mcfunction).
...
- Additional `SURVIVALGAMES.*` objectives can be created to suit the game's needs, just make sure they're namespaced with the `SURVIVALGAMES.` prefix.
...
- `SURVIVALGAMES.tp.arena` marker in the game arena. Everyone gets TPed here on game start (`function survivalgames:states/pregame/start`)
- `SURVIVALGAMES.tp.lobby` marker in the main lobby. Everyone gets TPed here on game end (`function survivalgames:exit`)
...
2. The datapack with the corresponding ID (defined in `id SURVIVALGAMES.config`) runs, starting with the `survivalgames:states/lobby/start` function, since the `?state` variable should be pre-configured to `-1`.
3. It then sets `?state` to `0`, activating `survivalgames:states/lobby/tick`. This function ticks down until the game begins, where it then calls `survivalgames:states/pregame/start`...
...
> [!WARNING]  
> Advancements will trigger even if the current gamestate doesn't match. Always check if `?state SURVIVALGAMES.game` matches in the reward function, and stop executing if it doesn't match.
>
...
- ALL tags must be namespaced (prefixed) with `SURVIVALGAMES.`.
